...
# Database
def addentry(Name, Password, Email, Address, City, Contact):
    db = sqlite3.connect("LibManagment.db")
    cur = db.cursor()
    cur.execute('''CREATE TABLE IF NOT EXISTS Add_lib2 ( Name text,Password text,Email text, 
                   Address text, City text, Contact int)''')
    db.commit()
    cur.execute('INSERT INTO Add_lib2 VALUES (?, ?, ?, ?, ?, ?);', (Name, Password, Email, Address, City, Contact))
...
class Addlib(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button1 = tk.Button(self, text="Add Libraian",
                            command=lambda: addentry(namevar.get(), pwdvar.get(), emailvar.get(), addressvar.get(),
                                                     cityvar.get(), contectvar.get()))
        button1.pack()
